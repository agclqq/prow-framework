...
func (c *Client) Watch(key string, f confctr.CallBack, option *confctr.WatchOption) error {
	...
	plan.Handler = func(idx uint64, val interface{}) {
		...
		case *consulapi.KVPair:
			f(string(option.Type), v.Key, fmt.Sprintf("%d %d %d %s", time.Now().UnixNano(), v.CreateIndex, v.ModifyIndex, string(v.Value)))
		case consulapi.KVPairs:
			for _, kv := range v {
				f(string(option.Type), kv.Key, fmt.Sprintf("%d %d %d %s", time.Now().UnixNano(), kv.CreateIndex, kv.ModifyIndex, string(kv.Value)))
			}
		// services
